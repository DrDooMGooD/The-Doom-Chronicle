...
export default function DoomIntro({ onComplete }: DoomIntroProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-stone-950 flex flex-col items-center justify-center overflow-y-auto p-4 font-mono select-none">
      ...
      <div className={`absolute inset-0 border-8 pointer-events-none transition-colors duration-500 ${
      ...
      <AnimatePresence>
        {!isZooming && (
          <motion.div
            ...
            className="w-full max-w-5xl flex flex-col items-center text-center px-4 mb-6"
          >
      ...
      <div className="w-full max-w-5xl grid grid-cols-1 md:grid-cols-12 gap-6 items-stretch z-10 my-4">
        
        {/* Left Side: The Sovereign Portrait (Zooming/Interactive Image) */}
        <div className="md:col-span-5 flex flex-col justify-center min-h-[300px] md:min-h-0">
          <motion.div
            animate={isZooming ? {
              scale: 45,
          ...
          <AnimatePresence mode="wait">
            {!isZooming && (
              <motion.div
                ...
                exit={{ opacity: 0, x: -20 }}
                className="flex-grow flex flex-col justify-between bg-stone-900 border-4 border-black p-6 shadow-comic relative rounded-lg"
              >
                {/* Supreme Stamp */}
                <div className="absolute -top-3.5 right-4 bg-emerald-600 border-2 border-black text-white font-comic text-[10px] uppercase px-3 py-1 rotate-[2deg] shadow-comic flex items-center space-x-1.5">
                  <Brain className="w-3.5 h-3.5" />
                  <span>INTELLECT CONFIRMATION</span>
                </div>

                <div className="space-y-4">
                  <h3 className="font-comic text-lg uppercase text-emerald-400 tracking-wide">
                  ...
                  <div className="min-h-[50px] flex items-center">
                    {clickMessage ? (
                      ...
                      <div className="w-full bg-emerald-950/50 border border-emerald-500 text-emerald-400 text-xs px-3 py-2 flex items-center space-x-2 rounded animate-pulse">
                        ...
                        <span className="font-bold uppercase">CORRECT! ACCESS GRANTED TO THE DOMAIN. PORTAL OPENING...</span>
                      </div>
                    ) : triviaStatus === 'incorrect' ? (
                      <div className="w-full bg-rose-950/50 border border-rose-600 text-rose-400 text-xs px-3 py-2 flex items-center space-x-2 rounded">
                        ...
                        <span className="font-bold uppercase">INCORRECT! DOOM REJECTS THIS FOOLISH ANSWER. TRY AGAIN!</span>
                      </div>
                    ) : (
                  ...
                  {/* Trivia Question Box */}
                  <div className="bg-stone-950 border-2 border-black p-4 rounded relative">
                    <p className="font-comic text-stone-100 text-sm md:text-base leading-snug">
                    ...
                    {currentQuestion.options.map((option) => {
                      ...
                      return (
                        <button
                          ...
                          className={`p-3 rounded text-left text-xs uppercase tracking-wider font-bold transition-all cursor-pointer flex items-center justify-between ${btnClass}`}
                        >
                          <span>{option}</span>
                          {isSelected && triviaStatus === 'correct' && <CheckCircle className="w-4 h-4 text-emerald-400 shrink-0" />}
                          {isSelected && triviaStatus === 'incorrect' && <XCircle className="w-4 h-4 text-rose-400 shrink-0" />}
                        </button>
                      );
                    })}
                  </div>
                ...
                <div className="mt-6 pt-4 border-t border-stone-800 flex items-center justify-between gap-4">
                  <button
                    ...
                    className="text-[10px] text-stone-400 hover:text-emerald-400 flex items-center space-x-1.5 transition-colors cursor-pointer uppercase"
                  >
                  ...
                  {triviaStatus === 'incorrect' && (
                    <button
                      ...
                      className="text-[10px] bg-stone-950 hover:bg-stone-850 text-emerald-400 hover:text-emerald-300 border border-emerald-950 hover:border-emerald-700 px-3 py-1.5 flex items-center space-x-1 transition-colors cursor-pointer uppercase"
                    >
                ...
                {/* Hint Text Display */}
                {showHint && !isZooming && (
                  <div className="mt-3 bg-stone-950/90 border border-emerald-900 p-2.5 text-[11px] text-stone-300 font-sans leading-relaxed">
      ...
      <AnimatePresence>
        {!isZooming && (
          <motion.div
            ...
            transition={{ duration: 0.5 }}
            className="w-full max-w-lg flex flex-col items-center text-center px-4 mt-4 space-y-5"
          >
            {/* Supreme Proclamation Quote card */}
            <div className="bg-stone-900 border-4 border-black p-4 shadow-comic relative max-w-md rounded-lg">
              <h1 className="font-comic text-xl sm:text-2xl text-white uppercase tracking-wider mb-1 leading-tight">
                THE DOOM CHRONICLE
              </h1>
              <p className="font-sans font-medium text-stone-300 text-[11px] sm:text-xs leading-relaxed">
              ...
              <p className="text-[9px] text-emerald-400 font-bold font-mono mt-2 uppercase tracking-widest text-center">
      ...
      {!isZooming && (
        <div className="absolute bottom-4 left-4 text-[9px] font-mono text-stone-600 uppercase flex items-center space-x-1.5 select-none">
